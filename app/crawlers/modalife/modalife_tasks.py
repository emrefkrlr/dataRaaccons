...
from crawlers.modalife import modalife_crawler
import datetime
import time


class ModalifeTasks(object):

    def modalife_crawler_tasks(self, activity_name):

        try:

            get_crawler_config = CrawlerServices().get_crawler_config(company="modalife", activity=activity_name)

            get_crawlers = CrawlerServices().fetch_urls_to_crawl({"status": 1, 'company__name': 'modalife', "activity__name": activity_name})
            ...
            for crawler in get_crawlers:
                ...
                if crawler.page_numbers >= 1:
                    for page in range(1, crawler.page_numbers + 1):
                        ...
                        innerHTML = modalife_crawler.ModaLifeCrawler().get_innerHTML(crawler.page_url, css_selector, page)
                        products_and_price = modalife_crawler.ModaLifeCrawler().html_parser(innerHTML, get_crawler_config, crawler.page_category)
                    ...
                    innerHTML = modalife_crawler.ModaLifeCrawler().get_innerHTML(crawler.page_url, css_selector)
                    products_and_price = modalife_crawler.ModaLifeCrawler().html_parser(innerHTML, get_crawler_config, crawler.page_category)
                ...
                if data['products_and_price']:
                    ...
                    if document_save:
                        print("Document saved mongodb Modalife...", datetime.datetime.utcnow())

        except Exception as e:
            print("ModalifeTasks modalife_crawler_tasks EXCEPTION: {}".format(e))