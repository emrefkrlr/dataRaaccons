...
class KilimMobilyaCrawler(object):
    ...
    def get_innerHTML(self, url, css_selector, page=None):
        ...
        try:
            ...
            print("KilimMobilyaCrawler get_innerHTML EXCEPTION: {}".format(e))
    ...
    def html_parser(self, html, crawler_config, page_category):
        ...
        try:
            ...
            for product in product_list:
                ...
                price = articlePrice.findChildren(eval(p5) , recursive=False)
                articleURL = product.find(eval(p6[0]), eval(p6[1]))

                # Replace key character with value character in string
                articlePrice = functions.char_to_replace(price[0].text.strip())
                image = functions.get_image_on_css(articleImage['style'], "https://kilimmobilya.com.tr")

                product_detail = {
                    'product_id': str(uuid.uuid4().hex),
                    'category': page_category,
                    'product_name': articleName.text.strip() if articleName.text != "" else None,
                    'product_url': 'https://kilimmobilya.com.tr' + articleURL['href'] if articleURL else None,
                    'measurement_value': None,
                    'currenct_unit': 'tl',
                    'price': float(articlePrice if articlePrice != "" else None),
                    'image': image if articleImage else None
                    }
            ...
            print("KilimMobilyaCrawler html_parser EXCEPTION: {}".format(e))