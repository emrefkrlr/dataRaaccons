...
from crawlers.migros import migros_crawler
import datetime
import time
...
class MigrosTasks(object):
    ...
    def migros_sanal_market_crawler_tasks(self, activity_name):

        try:

            get_crawler_config = CrawlerServices().get_crawler_config(company="migros_sanal_market", activity=activity_name)

            get_crawlers = CrawlerServices().fetch_urls_to_crawl({"status": 1, 'company__name': 'migros_sanal_market', "activity__name": activity_name})
