...
from crawlers.dogtas import dogtas_crawler
import datetime
import time


class DogtasTasks(object):

    def dogtas_crawler_tasks(self, activity_name):

        try:

            get_crawler_config = CrawlerServices().get_crawler_config(company="dogtas", activity=activity_name)

            get_crawlers = CrawlerServices().fetch_urls_to_crawl({"status": 1, 'company__name': 'dogtas', "activity__name": activity_name})
            ...
            for crawler in get_crawlers:
                ...
                if crawler.page_numbers >= 1:
                    for page in range(1, crawler.page_numbers + 1):
                        ...
                        innerHTML = dogtas_crawler.DogtasCrawler().get_innerHTML(crawler.page_url, css_selector, page)
                        products_and_price = dogtas_crawler.DogtasCrawler().html_parser(innerHTML, get_crawler_config, crawler.page_category)
                    ...
                    innerHTML = dogtas_crawler.DogtasCrawler().get_innerHTML(crawler.page_url, css_selector)
                    products_and_price = dogtas_crawler.DogtasCrawler().html_parser(innerHTML, get_crawler_config, crawler.page_category)
                ...
                if data['products_and_price']:
                    ...
                    if document_save:
                        print("Document saved mongodb Doğtaş...", datetime.datetime.utcnow())

        except Exception as e:
            print("DogtasTasks dogtas_crawler_tasks EXCEPTION: {}".format(e))