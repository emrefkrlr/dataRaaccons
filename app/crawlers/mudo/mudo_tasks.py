...
from crawlers.mudo import mudo_crawler
import datetime
import time


class MudoTasks(object):

    def mudo_crawler_tasks(self, activity_name):

        try:

            get_crawler_config = CrawlerServices().get_crawler_config(company="mudo", activity=activity_name)

            get_crawlers = CrawlerServices().fetch_urls_to_crawl({"status": 1, 'company__name': 'mudo', "activity__name": activity_name})
            ...
            for crawler in get_crawlers:
                ...
                if crawler.page_numbers >= 1:
                    for page in range(1, crawler.page_numbers + 1):
                        ...
                        innerHTML = mudo_crawler.MudoCrawler().get_innerHTML(crawler.page_url, css_selector, page)
                        products_and_price = mudo_crawler.MudoCrawler().html_parser(innerHTML, get_crawler_config, crawler.page_category)
                    ...
                    innerHTML = mudo_crawler.MudoCrawler().get_innerHTML(crawler.page_url, css_selector)
                    products_and_price = mudo_crawler.MudoCrawler().html_parser(innerHTML, get_crawler_config, crawler.page_category)
