...
from crawlers.ocasso import ocasso_crawler
import datetime
import time


class OcassoTasks(object):

    def ocasso_crawler_tasks(self, activity_name):

        try:

            get_crawler_config = CrawlerServices().get_crawler_config(company="ocasso", activity=activity_name)

            get_crawlers = CrawlerServices().fetch_urls_to_crawl({"status": 1, 'company__name': 'ocasso', "activity__name": activity_name})
            ...
            for crawler in get_crawlers:
                ...
                if crawler.page_numbers >= 1:
                    for page in range(1, crawler.page_numbers + 1):
                        ...
                        innerHTML = ocasso_crawler.OcassoCrawler().get_innerHTML(crawler.page_url, css_selector, page)
                        products_and_price = ocasso_crawler.OcassoCrawler().html_parser(innerHTML, get_crawler_config, crawler.page_category)
                    ...
                    innerHTML = ocasso_crawler.OcassoCrawler().get_innerHTML(crawler.page_url, css_selector)
                    products_and_price = ocasso_crawler.OcassoCrawler().html_parser(innerHTML, get_crawler_config, crawler.page_category)
