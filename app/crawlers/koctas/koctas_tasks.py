...
from crawlers.koctas import koctas_crawler
import datetime
import time


class KoctasTasks(object):

    def koctas_crawler_tasks(self, activity_name):

        try:

            get_crawler_config = CrawlerServices().get_crawler_config(company="koctas", activity=activity_name)

            get_crawlers = CrawlerServices().fetch_urls_to_crawl({"status": 1, 'company__name': 'koctas', "activity__name": activity_name})
            ...
            for crawler in get_crawlers:
                ...
                if crawler.page_numbers >= 1:
                    for page in range(1, crawler.page_numbers + 1):
                        ...
                        innerHTML = koctas_crawler.KoctasCrawler().get_innerHTML(crawler.page_url, css_selector, page)
                        products_and_price = koctas_crawler.KoctasCrawler().html_parser(innerHTML, get_crawler_config, crawler.page_category)
                    ...
                    innerHTML = koctas_crawler.KoctasCrawler().get_innerHTML(crawler.page_url, css_selector)
                    products_and_price = koctas_crawler.KoctasCrawler().html_parser(innerHTML, get_crawler_config, crawler.page_category)
                ...
                if data['products_and_price']:
                    ...
                    if document_save:
                        print("Document saved mongodb Koçtaş...", datetime.datetime.utcnow())

        except Exception as e:
            print("KoctasTasks koctas_crawler_tasks EXCEPTION: {}".format(e))